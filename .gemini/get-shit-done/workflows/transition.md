...
```bash
TRANSITION=$(node "C:/DEVIL'S ADVOCATE/.gemini/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "C:/DEVIL'S ADVOCATE/.gemini/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
node "C:/DEVIL'S ADVOCATE/.gemini/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
