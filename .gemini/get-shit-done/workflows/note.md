...
- **Global scope**: `C:/DEVIL'S ADVOCATE/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/DEVIL'S ADVOCATE/.gemini/notes/`)
...
2. Glob `C:/DEVIL'S ADVOCATE/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (C:/DEVIL'S ADVOCATE/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/DEVIL'S ADVOCATE/.gemini/notes/` — works in any directory
